defmodule Rumbl.VideoControllerTest do
  ...
  @tag login_as: "max"
  test "creats user video and redirects", %{conn: conn, user: user} do 
  ...
  video = insert_video(owner, @valid_attrs) non_owner = insert_user(username: "sneaky") conn = assign(conn, :current_user, non_owner)
  assert_error_sent :not_found, fn -> get(conn, video_path(conn, :show, video))
  end
  ...
  assert_error_sent :not_found, fn ->
  delete(conn, video_path(conn, :delete, video)) end
  end

  @tag login_as: "max"
  test "does not create video and renders errors when invalid", %{conn: conn} do
  count_before = video_count(Video)
  conn = post conn, video_path(conn, :create), video: @invalid_attrs assert html_response(conn, 200) =~ "check the errors"
